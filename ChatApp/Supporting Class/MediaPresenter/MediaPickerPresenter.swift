...
public struct AttachmentManager {
    ...
    public struct Settings {
        ...
        public var allowedAttachments: [AttachmentType] = [.camera, .photoLibrary]
    }
    ...
    fileprivate mutating func prepareImagePicker(for type: AttachmentType) {
        ...
        case .video:
            imageHandler = ImagePickerHandler(sourceType: .savedPhotosAlbum, allowsEditing: settings.libraryAllowsEditing)
        default:break
        }
    }
...
public extension MediaPickerPresenter {
        ...
        for attachment in allowedAttachments {
            ...
            case .video:
                attachmentManager.actionSheet.addAction(UIAlertAction(title: titles.video, style: .default, handler: { (action) in
                    self.checkAuthorizationStatus(attachmentType: .video)
                    //self.openVideo()
                }))
            }
        }
    ...
    private func openVideo() {
        ...
        if UIImagePickerController.isSourceTypeAvailable(.photoLibrary) {
            attachmentManager.prepareImagePicker(for: .video)
            attachmentManager.imageHandler?.delegate = self
            self.attachmentManager.imageHandler?.picker.mediaTypes = ["public.movie"]
            DispatchQueue.main.async {
                self.viewController.present((self.attachmentManager.imageHandler?.picker)!, animated: true, completion: nil)
            }
        }
